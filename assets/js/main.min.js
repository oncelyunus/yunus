$(document).ready(function () {
  ...
  $('#langSelector').change(function() {
    ...
    const redirectToMarket = () => {
        if (isIOS()) {
            window.location = "https://itunes.apple.com/en/app/fizy-music-video/id404239912?mt=8"
        }
        if (isAndroid()) {
            window.location = "https://play.google.com/store/apps/details?id=com.turkcell.gncplay&hl=en"
        }
    };

    const redirectToFizy = () => {
        window.location = "http://play.fizy.com/"
    };

    const openMobileApp = () => {
        const isDocumentHidden = () => document.hidden || document.visibilityState === 'hidden' || document['webkitHidden'];
        alert('isDocumentHidden '+ isDocumentHidden());
        let timeoutId;
        ...
        alert('burada 2');
        window.location = `fizy://listen.fizy.com/timeline`;
        alert('burada 3');
    };
